...
# Check Arrays:
echo "SUBJECTS_VEC: " ${subjects[*]}
echo "RUNS_VEC: " ${runs[*]}
echo "COUNT_PARALLEL: " $N_COUNT
#exit
...
####################################
# Register b_anat to ICBM-152
# Fairly quick, ~30 min. each
####################################
for i in $(seq -f "%02g" 1 $N_SUBJECTS)
do
	# Limit Number of Jobs Submitted to MAX_JOBS:
	...
	# Make sure file exists
	(if [ ! -f b_sub-${i}_anat.nii ]; then
		echo b_sub-${i}_anat.nii "does not exist"
    		continue
	fi
	${ANTSPATH}/antsRegistrationSyN.sh -d 3 -f mni_temp.nii -m b_sub-${i}_anat.nii -n 1 -t s -o MNI_sub-${i}_)&
done
wait
...
do
	...
	${ANTSPATH}/antsApplyTransforms -d 3 -o [CollapsedWarp_sub-${i}_run-${j}_bold.nii.gz,1] \
		...
		-t MNI_sub-${i}_1Warp.nii.gz \
		-t MNI_sub-${i}_0GenericAffine.mat \
		-t coreg_sub-${i}_run-${j}_0GenericAffine.mat \
		-r rmni_temp.nii

	# Inputs: Dimension, Output, ReplicateDisplacement VectorFieldName TimeDims TimeSpacing TimeOrigin
